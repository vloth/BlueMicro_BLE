...
arduino_compile() {
   ...
   #Compile
   cmdCompile832="/usr/local/bin/arduino-cli compile -v --fqbn adafruit:nrf52:feather52832 --build-path $buildPath --build-cache-path $buildCachePath $sourcePath/firmware.ino"
   cmdCompile840="/usr/local/bin/arduino-cli compile -v --fqbn adafruit:nrf52:pca10056:softdevice=s140v6,debug=l0 --build-path $buildPath --build-cache-path $buildCachePath $sourcePath/firmware.ino"
